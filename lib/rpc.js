...
import { readNetwork, scoreNetwork, explainScore, SCORING_VERSION, KNOWN_COMPANIES } from './scoring.js';
...
import { sectorMatcher, sectorLabel } from './sector-directory.js';

// The curated company list the stored scores were computed with: a fingerprint
// of every entry (name, score, alias, industry), stamped beside them in
// app_meta 'scoring_list'. Editing the list makes stored scores stale, as a new
// SCORING_VERSION does, with no number to remember to bump. Copies from before
// the list was made neutral stamped none, which is also how rescoreAll() knows
// to offer keeping their old scores (lib/legacy-offer.js).
export const KNOWN_LIST_STAMP = createHash('sha256')
  .update(JSON.stringify(KNOWN_COMPANIES.map(([name, score, alias, industry]) => [name, score, String(alias), industry])))
 ...
 * headcount, its one industry and the sector directory's sectors it matches.
 * Rescoring, Settings' preview and its suggestions all read the rows through
 * this, so they agree on which company is in which sector.
 */
export function readForScoring(rows) {
  return readNetwork(rows, { industryOf: industryKeyOf, sectorsOf: sectorMatcher() });
}
