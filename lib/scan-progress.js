...
//
// And one from the app itself (app/api/scraper/route.js setupPlan), while
// "Set up the scanner" downloads its Python:
//                "Downloading Python 3.12.14: 12.0 of 24.0 MB"

const REPORTED = /LinkedIn reports (\d+) connections/;
const DOWNLOADING = /^Downloading Python [\d.]+: ([\d.]+) of ([\d.]+) MB$/;
...
export function scanProgress(log, action) {
  ...
  // Only while the download is the step under way: once the log has moved on
  // (unpacking, installing), a bar left at 100% would look stuck.
  if (action === 'setup') {
    const m = String(log[log.length - 1]).match(DOWNLOADING);
    if (!m || !Number(m[2])) return null;
    return { done: Math.min(Number(m[1]), Number(m[2])), total: Number(m[2]), kind: 'download' };
  }
