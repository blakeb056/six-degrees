...
// the dry run the page shows before you save (which a save also reports).
//
 ...
 * `industryOf` the injected inference, as rescoreAll passes them; `read` is
...
export function previewSectorFocus(rows = [], {
  overrides = new Map(), industryOf, from = NO_FOCUS, to = NO_FOCUS, examples = 5, read = readNetwork(rows, { industryOf }),
} = {}) {
  ...
  for (const [name, a] of before.companyScores) {
    ...
    changed.push({ name, from: a.score, to: b.score, sector: read.companies.get(name)?.industry, people: read.headcount.get(name) || 0 });
  }
