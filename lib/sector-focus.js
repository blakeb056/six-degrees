...
// its one industry or by any of the industry's sectors, or a narrower sector
// from the directory (lib/sector-directory.js), which it matches by the
// directory's words. The
...
import { SECTOR_KEYS, DIRECTORY_VERSION, sectorMatcher, sectorGroup } from './sector-directory.js';
 ...
 * recomputed on the next load. Every pick matches by the directory's word
 * lists (a broad industry includes its sectors), so the fingerprint carries
 * the directory's version: "lean:health,dental@<version>". Stamps from before
 * a broad industry included its sectors ("lean:media,tech") no longer match,
 * so those scores are redone once. `version` is for tests.
 */
export function focusFingerprint(focus, { version = DIRECTORY_VERSION } = {}) {
  ...
  return `${f.strength}:${f.sectors.join(',')}@${version}`;
}
 ...
 * `industryOf`, `sectorsOf` and `groupOf` the injected inference and sector
 * directory, as rescoreAll passes them (lib/rpc.js readForScoring); `read` is
 * readNetwork() of these rows, when they have been read already.
...
export function previewSectorFocus(rows = [], {
  overrides = new Map(), industryOf, sectorsOf = sectorMatcher(), groupOf = sectorGroup, from = NO_FOCUS, to = NO_FOCUS, examples = 5,
