import { readTitle, currentCompany, rolesWithCompanies, cleanCompany, companyIndustry, networkCompanies } from './scoring.js';
...
export function normalizeCompany(name) {
  ...
  // Bain Capital is a private equity firm, not the consultancy (scoring keeps
  // them apart too, so they don't share an industry).
  if (n.includes('bain') && !n.includes('bain capital')) return 'Bain & Company';
...
/** The company someone works at now: a company scan's, else their current role's (lib/scoring.js). */
export function companyOf(c) {
  const co = currentCompany(c);
  return co ? normalizeCompany(co) || co : null;
}
...
// are the same for everyone: no one school, region, job or company of one
// person's. "UCF", "Snap", "Lens" and "AR" were ("AR Specialist" is accounts
// receivable), and so were Polymarket, Anduril, Sandia and Whatnot ("whatnot"
// is also a word), and "growth", a job title, which made "Head of Growth at
// Acme Software" and "Summit Growth Equity" media. Editing them changes the
// stored scores' list stamp (lib/rpc.js), since a company's one industry can
// decide its estimate and your sector.
 ...
 * Settings → Your sector leans on. Paths merges a few names scoring keeps
 * apart (normalizeCompany: "BNY" is "BNY Mellon" here), so a name is also
 * looked up as scoring would write it.
 */
export function buildCompanyIndex(rows = []) {
  ...
  for (const r of best.values()) {
    ...
    const current = rolesOf(r).find((x) => !x.former)?.company;
    const name = current ? normalizeCompany(current) || current : null;
    if (!name) continue;
  ...
  for (const co of index.values()) {
    const key = industries.get(co.name) || industries.get(cleanCompany(co.name))
      || companyIndustry(co.name, { industryOf: industryKeyOf, headlines: co.people.map((p) => p.headline) });
    co.industry = industryByKey(key);
  }
  return index;
}
