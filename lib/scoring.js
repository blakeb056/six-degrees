...
// (Ford, Delta, Apollo, Columbia) matches only in the forms its company uses,
// and never as a short form that is also a word or a title: "GM" is a general
// manager, and "Honda of …" a dealership with its own staff.
//
...
const VENUE = String.raw`(?! (arena|stadium|center|centre|field|park|amphitheat(er|re)|pavilion|forum|coliseum|speedway)\b)`;
...
export const KNOWN_COMPANIES = [
  ...
  ['Merck', 8, /^merck\b(?! (kgaa|group)\b)/, 'health'], ['AbbVie', 8, /^abbvie\b/, 'health'], ['Eli Lilly', 8, /^eli lilly\b|^lilly( usa)?$/, 'health'],
  ...
  // system of its own.
  ['Princeton University', 8, /^princeton( university\b|$)/, 'education'], ['Yale University', 8, /^yale( university\b| school of\b| law school\b|$)/, 'education'],
  ...
  ['Northwestern University', 7, /^northwestern( university\b|$)/, 'education'], ['University of Pennsylvania', 7, /^(university of pennsylvania\b|upenn\b|penn$)/, 'education'],
  ['Cornell University', 7, /^cornell( university\b| tech$|$)/, 'education'], ['University of Chicago', 7, /^(university of chicago\b|uchicago$)/, 'education'],
  ['Brown University', 7, /^brown( university\b|$)/, 'education'], ['Columbia University', 7, /^columbia (university|business school|law school)\b/, 'education'],
  ['Dartmouth College', 7, /^dartmouth( college\b|$)/, 'education'], ['UCLA', 7, /^(ucla|university of california,? los angeles)$|^ucla anderson\b/, 'education'],
  ['Rice University', 7, /^rice university\b/, 'education'], ['University of Notre Dame', 7, /^(university of notre dame\b|notre dame$)/, 'education'],
  ['Vanderbilt University', 7, /^vanderbilt( university\b|$)/, 'education'],
];

const JUNK_COMPANY = /^(self[-\s]?employed|freelance|various|multiple|confidential|n\/a|none|tbd|open to work|looking|seeking)$/i;
// "at scale", "at the intersection of…", "at heart", "at home": phrases, not
// employers. Home Depot is one.
const NOT_A_COMPANY = /^(scale$|heart|large|night|home(?! depot\b)|work|speed|best|the intersection|intersection|the crossroads|the forefront|the core|every stage|all levels|your service)\b/i;
