...
export const KNOWN_COMPANIES = [
  ...
  ['Bain', 9, /^bain( & company| and company| capital)?$/, 'consulting'],
 ...
 * `focus` is the sector focus from Settings, or nothing. Returns one result
 * per row id, plus the per-company facts it used.
 */
export function scoreNetwork(rows = [], { overrides = new Map(), industryOf, focus } = {}) {
  const { headcount, industries } = networkCompanies(rows, { industryOf });
  ...
  const formerOnly = new Map();
  const industryFor = (name) => {
    if (industries.has(name)) return industries.get(name);
    if (!formerOnly.has(name)) formerOnly.set(name, companyIndustry(name, { industryOf }));
    return formerOnly.get(name);
  };
  const out = new Map();
  for (const r of rows) {
    const companyFor = (name) => companyScore(name, { overrides, headcount: headcount.get(name) || 0, industry: name ? industryFor(name) : undefined, focus });
  ...
  return { scores: out, headcount, industries };
}
