...
export const KNOWN_COMPANIES = [
  ...
  ['Pinterest', 8, /^pinterest\b/, 'media'], ['Reddit', 8, /^reddit\b/, 'media'], ['X', 8, /^(x corp|twitter)\b/, 'media'], ['Roblox', 8, /^roblox\b/, 'entertainment'],
  ['Shopify', 8, /^shopify\b/, 'consumer'], ['Databricks', 8, /^databricks\b/, 'tech'], ['Snowflake', 8, /^snowflake\b/, 'tech'], ['Datadog', 8, /^datadog\b/, 'tech'],
  ['CrowdStrike', 8, /^crowdstrike\b/, 'tech'], ['ServiceNow', 8, /^servicenow\b/, 'tech'], ['Figma', 8, /^figma\b/, 'tech'], ['Canva', 8, /^canva\b/, 'tech'],
  ['Scale AI', 8, /^(scale ai|scale\.ai|scale\.com)$/, 'tech'], ['Intuit', 8, /^intuit\b/, 'tech'], ['Cisco', 8, /^cisco\b/, 'tech'], ['Qualcomm', 8, /^qualcomm\b/, 'tech'], ['AMD', 8, /^amd\b/, 'tech'],
  ['Samsung', 8, /^samsung\b/, 'tech'], ['Sony', 8, /^sony\b/, 'entertainment'], ['Epic Games', 8, /^epic games\b/, 'entertainment'], ['Whatnot', 8, /^whatnot\b/, 'consumer'],
  ...
  ['Accenture', 8, /^accenture\b/, 'consulting'], ['Egon Zehnder', 8, /^egon zehnder\b/, 'consulting'], ['Heidrick & Struggles', 8, /^heidrick\b/, 'consulting'],
  ...
  ['NBCUniversal', 8, /^(nbcuniversal|nbc universal|nbc)\b/, 'entertainment'], ['Warner Bros. Discovery', 8, /^(warner|wbd)\b/, 'entertainment'], ['MrBeast', 8, /^(mrbeast|beast industries)\b/, 'media'],
  ...
  ['Discord', 7, /^discord\b/, 'tech'], ['Whop', 7, /^whop\b/, 'tech'], ['Hims & Hers', 7, /^hims\b/, 'health'], ['Paramount', 7, /^paramount\b/, 'entertainment'],
  ['Madison Square Garden', 7, /^(madison square garden|msg)\b/, 'entertainment'], ['Fanatics', 7, /^fanatics\b/, 'consumer'], ['Nielsen', 7, /^nielsen\b/, 'media'],
  ['The Trade Desk', 7, /^the trade desk\b/, 'media'], ['SiriusXM', 7, /^siriusxm\b/, 'entertainment'], ['Riot Games', 7, /^riot games\b/, 'entertainment'], ['Twitch', 7, /^twitch\b/, 'entertainment'],
  ['Electronic Arts', 7, /^(electronic arts|ea)$/, 'entertainment'], ['Activision Blizzard', 7, /^(activision|blizzard)\b/, 'entertainment'], ['Lyft', 7, /^lyft\b/, 'tech'],
  ['Instacart', 7, /^instacart\b/, 'tech'], ['Duolingo', 7, /^duolingo\b/, 'tech'], ['Notion', 7, /^notion\b/, 'tech'], ['Vercel', 7, /^vercel\b/, 'tech'],
  ['Intel', 7, /^intel( corporation)?$/, 'tech'], ['Workday', 7, /^workday\b/, 'tech'], ['PayPal', 7, /^paypal\b/, 'finance'], ['Plaid', 7, /^plaid\b/, 'finance'],
  ['Brex', 7, /^brex\b/, 'finance'], ['Ramp', 7, /^ramp$/, 'finance'], ['Mercury', 7, /^mercury$/, 'finance'], ['Chime', 7, /^chime$/, 'finance'], ['Credit Karma', 7, /^credit karma\b/, 'finance'],
  ['Chick-fil-A', 7, /^chick[-\s]fil[-\s]a\b/, 'consumer'], ['Target', 7, /^target( corporation)?$/, 'consumer'], ['Booz Allen Hamilton', 7, /^booz allen\b/, 'defense'],
  ['L3Harris', 7, /^l3harris\b/, 'defense'], ['Leidos', 7, /^leidos\b/, 'defense'], ['U.S. Space Force', 7, /^(u\.?s\.? )?space force\b/, 'defense'],
  ...
  // 6 — notable
  ['Grindr', 6, /^grindr\b/, 'tech'], ['Genius Sports', 6, /^genius sports\b/, 'entertainment'], ['Later', 6, /^later$/, 'media'], ['Hard Rock Digital', 6, /^hard rock\b/, 'entertainment'],
  ['Vanta', 6, /^vanta\b/, 'tech'], ['Kaseya', 6, /^kaseya\b/, 'tech'], ['Havas', 6, /^havas\b/, 'media'], ['VaynerMedia', 6, /^vayner/, 'media'],
  ['AdventHealth', 6, /^adventhealth\b/, 'health'], ['University of Florida', 6, /^(uf|university of florida)$/, 'education'], ['The Athletic', 6, /^the athletic\b/, 'media'],
 ...
 * it, each company also gets the directory's sectors it matches.
 * @returns { headcount: Map name → n, industries: Map name → industry key, sectors: Map name → sector keys }
 */
export function networkCompanies(rows = [], { industryOf, sectorsOf, rolesOf = rolesWithCompanies } = {}) {
  const at = new Map();
  const seen = new Set();
  for (const r of rows) {
  ...
  for (const [name, c] of at) {
    ...
    if (sectorsOf) sectors.set(name, sectorsOf(name, c.headlines));
  }
  return { headcount, industries, sectors };
}
 ...
 * key or a key from the sector directory. A broad industry matches the
 * company's one `industry`; a directory sector matches when it is among the
 * company's `sectors` (the directory's matches, injected). However many
 * match, the bonus is added once; `sector` names the pick that matched, a
 * directory sector before a broad industry. Capped at 10. The extra fields
 * appear only when the score actually moved, so everything that reads a
 * plain { score, source } keeps working.
 */
function leanToward(base, industry, sectors, focus) {
  ...
  const pick = (sectors?.length && focus.sectors.find((k) => sectors.includes(k)))
    || (industry && focus.sectors.includes(industry) ? industry : null);
  if (!pick) return base;
 ...
 * directory's keys it matches (lib/sector-directory.js), when known.
 */
export function companyScore(name, { overrides = new Map(), headcount = 0, industry, sectors, focus } = {}) {
  ...
  return focus ? leanToward(base, ind, sectors, focus) : base;
}
 ...
 * sectors it matches, worked out here once however many ways the read is
 * scored. Pass it to scoreNetwork() with the same rows.
 * @returns { people: Map row → readPerson, headcount, industries, companies: Map name → { headcount, industry, sectors? } }
 */
export function readNetwork(rows = [], { industryOf, sectorsOf } = {}) {
  const people = new Map();
  for (const r of rows) people.set(r, readPerson(r));
  const { headcount, industries, sectors } = networkCompanies(rows, { industryOf, sectorsOf, rolesOf: (r) => people.get(r).roles });
  const companies = new Map();
  for (const [name, n] of headcount) {
    companies.set(name, { headcount: n, industry: industries.get(name), ...(sectorsOf && { sectors: sectors.get(name) }) });
  }
  ...
  for (const { roles } of people.values()) {
    for (const { company } of roles) {
      if (company && !companies.has(company)) {
        companies.set(company, { headcount: 0, industry: companyIndustry(company, { industryOf }), ...(sectorsOf && { sectors: sectorsOf(company, []) }) });
      }
    }
  }
 ...
 */
export function scoreNetwork(rows = [], { overrides = new Map(), industryOf, sectorsOf, focus, read = readNetwork(rows, { industryOf, sectorsOf }) } = {}) {
  ...
  const companyFor = (name) => {
    if (!companyScores.has(name)) {
      ...
      companyScores.set(name, companyScore(name, { overrides, headcount: c?.headcount || 0, industry: c?.industry, sectors: c?.sectors, focus }));
    }
