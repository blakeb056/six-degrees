 ...
 * `scanRunning` is what the scanner's route runs (lib/scan-state.js
 * scannerJob: 'full', 'install', …), or null; the refusal names it.
...
export function importPreflight({ scanRunning, pending, declared, currentPeople = 0, confirmedPeople, max = MAX_IMPORT_BYTES }) {
  if (scanRunning) {
    return new ImportError(`${runningNow(scanRunning)}. Let it finish, or stop it on the Scan page, then import.`, { status: 409 });
  }
  if (pending) {
