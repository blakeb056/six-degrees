...
/**
 * choosePython() for a server that asks again and again (the Scan page asks
 * every 1.5 seconds), remembering how the Python it is told to use first
 * (`named`, SIX_DEGREES_PYTHON) went:
 *   - once it has worked it isn't started again to ask: it can't change while
 *     the server runs;
 *   - the app's own ('bundled') that didn't work isn't started again while this
 *     server runs. If macOS refused to run it (say, Gatekeeper blocking the
 *     Python inside an app installed from a quarantined download), every start
 *     could bring its "python3.12 Not Opened" alert back. Its failure is
 *     reported on every look instead, with retry: false, so the Scan page says
 *     what happened and what to do (lib/scanner-setup.js); a restart asks again;
 *   - one named from outside the app ('custom') that didn't work is asked again
 *     a minute later: someone may be fixing it. A first start can take 20
 *     seconds to give up, and every look would wait on it.
 * `options` go to choosePython as they are.
 */
export function pythonLooker({ choose = choosePython, now = Date.now, retryMs = 60000 } = {}) {
  let works = null;   // the named Python's `run`, once it has worked
  let failed = null;  // { path, at, own }
  return async function lookForPython(named, options = {}) {
    if (named && works?.path === named) {
      return { run: works, own: null, base: null, systemFound: null, download: null };
    }
    const knownBad = Boolean(named) && failed?.path === named
      && (failed.own.retry === false || now() - failed.at < retryMs);
    const python = await choose({ ...options, bundled: knownBad ? '' : named });
    if (knownBad) return { ...python, own: failed.own };
    if (named && python.run?.path === named) {
      works = python.run;
    } else if (named && python.own) {
      const own = python.own.source === 'bundled' ? { ...python.own, retry: false } : python.own;
      failed = { path: named, at: now(), own };
      return { ...python, own };
    }
    return python;
  };
