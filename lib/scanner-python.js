...
/**
 * The environment a Python the app ships (or names) runs with. It must use its
 * own packages and nothing of this user's: no user site-packages (a
 * `pip install --user` of another Playwright would come first on the path), no
 * PYTHONPATH or PYTHONHOME. And it never writes bytecode: inside the app that
 * would be writing into the app's own signed bundle.
 */
export function ownPythonEnv(env = process.env) {
  const out = { ...env, PYTHONNOUSERSITE: '1', PYTHONDONTWRITEBYTECODE: '1' };
  delete out.PYTHONPATH;
  delete out.PYTHONHOME;
  return out;
}
...
/** Private working folders for a setup, inside the data folder; swept once an hour old. */
...
export async function choosePython({
  ...
  if (bundled) {
    ...
    const r = await probe(bundled, ['-c', PROBE_SCRIPT], { timeoutMs: timeouts.own, env: ownPythonEnv() });
  ...
  if (exists(venv)) {
    const r = await probe(venv, ['-c', PROBE_SCRIPT], { timeoutMs: timeouts.other });
    const p = parseProbe(r.stdout);
    if (r.ok && p.imports) {
  ...
  for (const cmd of systemCandidates) {
    const r = await probe(cmd, ['-c', PROBE_SCRIPT], { timeoutMs: timeouts.other });
    const p = parseProbe(r.stdout);
  ...
  if (exists(own)) {
    const r = await probe(own, ['-c', PROBE_SCRIPT], { timeoutMs: timeouts.other, env: ownPythonEnv() });
    const p = parseProbe(r.stdout);
    if (r.ok && p.venv) {
