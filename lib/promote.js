...
export async function promoteToFirstDegree(db, { profileUrl, userId, fields = {} }) {
  ...
  if (direct.length > 0) {
    ...
    await repoint(db, fold.map((r) => r.id), keep.id);
  ...
  await repoint(db, fold.map((r) => r.id), source.id);
  for (const row of fold) {
    await db.from('linkedin_connections').delete().eq('id', row.id);
  }
...
/** Anything that pointed at a row about to be folded away points at the kept one. */
async function repoint(db, fromIds, toId) {
  for (const id of fromIds) {
    await db.from('linkedin_connections').update({ source_connection_id: toId }).eq('source_connection_id', id);
    await db.from('linkedin_connections').update({ unlocked_from_bridge_id: toId }).eq('unlocked_from_bridge_id', id);
  }
}
