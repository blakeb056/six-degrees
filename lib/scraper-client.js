...
export function notReadyMessage(status) {
  ...
  if (!status.checks?.scriptsFound) return 'The scanner files are missing from this install.';
  if (!status.checks?.python) return 'Python 3 is not installed on this machine.';
  if (!status.checks?.dependencies) return 'The scanner is not installed yet — open Scan to set it up.';
