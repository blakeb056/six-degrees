...
export function pausedList(firstDegree, mappedIds, progress = {}, unclear = {}) {
  ...
  // The scanner's order: dated people newest first, then the undated by when
  // they were saved; anyone unclear twice goes to the back. So the first people
  // shown are the first "Resume all" reads.
  const key = (x) => `${x.connected ? 1 : 0}|${x.connected || x.created || ''}`;
  out.sort((a, b) => (a.unclear >= 2) - (b.unclear >= 2) || key(b).localeCompare(key(a)));
  return out;
}
