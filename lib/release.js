...
/**
 * 'mac-app' or 'npm'. The launchers say which (SIX_DEGREES_INSTALL); an older
 * Mac app that predates that is recognised by where it is running from.
 */
export function installKind(env = process.env, root = '') {
  if (env.SIX_DEGREES_INSTALL === 'mac-app' || env.SIX_DEGREES_INSTALL === 'npm') {
