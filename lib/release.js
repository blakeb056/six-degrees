...
// else was installed — the Mac app (install.sh or the .dmg), the Windows app
// (install.ps1) or the npm package
...
/**
 * 'mac-app', 'win-app' or 'npm'. The launchers say which (SIX_DEGREES_INSTALL);
 * an older Mac app that predates that is recognised by where it is running from.
 */
export function installKind(env = process.env, root = '') {
  if (['mac-app', 'win-app', 'npm'].includes(env.SIX_DEGREES_INSTALL)) {
...
export function updateCommand(kind, slug) {
  ...
  if (kind === 'win-app') {
    return `irm https://raw.githubusercontent.com/${slug}/main/install.ps1 | iex`;
  }
