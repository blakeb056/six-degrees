...
const OLD = LEGACY_SCORES.map(([name, score, alias, industry]) => ({ name, score, alias, industry }));
 ...
 * names are found by its old alias, the way cleanCompany used to find them:
 * "University of Central Florida" is its own name now, and was UCF's.
 */
export function legacyEntryFor(name) {
  ...
  for (const entry of OLD) {
    ...
    if (entry.alias.test(lower) || entry.alias.test(bare) || lower === entry.name.toLowerCase()) return entry;
  }
  return null;
}

/** 'open', 'kept', 'declined', or null when this database was never offered. */
 ...
 * that you haven't replaced with one of your own.
 *
