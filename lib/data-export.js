...
import { AVATAR_FILE, TRAVELLING_FILES } from './data-folder.js';
 ...
 * if asked, the photos. Only plain files are taken, never a link, so nothing
 * outside the folder can be pulled in through one.
 */
export function travellingFiles(dir, { includePhotos = true } = {}) {
  const plain = (p) => {
  ...
  if (includePhotos) {
    let names = [];
    try { names = readdirSync(path.join(dir, 'avatars')).sort(); } catch { /* no photos yet */ }
    for (const name of names) {
      if (!AVATAR_FILE.test(name)) continue;
...
export function buildExport(db, { dir, outFile, includePhotos = true, appVersion, schemaSql, now = new Date() }) {
  ...
  try {
    ...
    let photos = 0;
    let files = 0;
    for (const f of travellingFiles(dir, { includePhotos })) {
      let bytes;
