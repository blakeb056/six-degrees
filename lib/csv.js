...
export function saveCsvNetwork(connections) {
  try {
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ degree1: connections, importedAt: Date.now() }));
    return true;
  } catch {
    return false; // quota — very large exports
  }
}
...
export function loadCsvNetwork() {
  try {
    ...
    return { degree1: parsed.degree1 || [], degree2: [] };
  } catch { return null; }
}
