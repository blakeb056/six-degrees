...
| `POST /api/data/import` | The body is the `.sixdegrees` file itself, with `X-Six-Degrees-Size` (its size: the bytes that arrive must match, so a body cut short can't pass for a whole one) and, when this copy already has people, `X-Six-Degrees-Replace: <how many>`, the count the user agreed to replace (a different count is a 409 with `needsConfirm`, and the page asks again). 409 while a scan runs or another import waits; 413 over 512 MB. Checks everything (`lib/data-import.js validateImport`), rebuilds the network into this version's schema in `import-pending/`, and answers `{ok, pending}`. **Nothing in the data folder changes until the next start**, when `getDb()` swaps it in. `DELETE` throws away an import that hasn't started. **Gated: it replaces the network.** |
...
Because `middleware.js` exists, Next buffers every `/api` request body so both it and
the route can read it, and by default it keeps only the first 10 MB: the rest is dropped
with a console warning and **no error**, and the route sees a shorter body. An import
(`POST /api/data/import`) is one file of about 5–25 MB, so `next.config.mjs` raises
`experimental.proxyClientMaxBodySize` to 512 MB, the most an import accepts
(`MAX_IMPORT_BYTES`), and the route still compares the bytes that arrived with
`X-Six-Degrees-Size`. Any new route that takes a large body needs the same check.
